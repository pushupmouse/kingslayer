...
public class StatIncreaseSelector : MonoBehaviour
{
    ...
    private Dictionary<KeyCode, StatIncreaseSelectorButton> _keyBindings;
    ...
    private void Update()
    {
        foreach (var key in _keyBindings.Keys)
        {
            ...
            {
                _keyBindings[key].Button.onClick.Invoke();
            }
        }
    }
    ...
    {
        _keyBindings = new Dictionary<KeyCode, StatIncreaseSelectorButton>();
        
        for (int i = 0; i < _statIncreaseSelectorButtons.Length; i++)
        {
            KeyCode key = KeyCode.Alpha1 + i;
            if ((int)key <= (int)KeyCode.Alpha9)
            {
                _keyBindings[key] = _statIncreaseSelectorButtons[i];
            }
        }
    }
        ...
        switch (rarity)
        {
            case RarityType.Legendary:
                return _legendaryStatIncreaseList.Count > 0 
                    ? new List<StatIncreaseData>(_legendaryStatIncreaseList) 
                    : new List<StatIncreaseData>(_epicStatIncreaseList); // Fallback to Epic
            case RarityType.Epic:
